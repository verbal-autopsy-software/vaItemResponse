...
#' 
#' @param icd10Code one ICD-10 codes (instead of a vector)
#' @examples
#' mapICD("A90")
#' @export
#' 
mapICD <- function(icd10Code){
    ...
    for (i in 1:nrow(icd10Map)) { # i = 1
        ...
        if (start <= numCode & numCode <= stop) {
            ...
            if(icd10Map[i, 1] == "Reproductive neoplasms (female)") icd10Map[i, 1] = "Respiratory neoplasms"
            if(icd10Map[i, 1] == "Reproductive neoplasms (male)") icd10Map[i, 1] = "Respiratory neoplasms"
            return(icd10Map[i, 1])
        }
    }
    return('Other (not in InterVA-5 cause list)')
}
